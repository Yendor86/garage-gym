...
function renderPicker(){
  ...
  if(!pickList){
    ...
    if(cl.length) cardioHtml='<div class="pickhead">❤️ Cardio</div>'+cl.map(function(c){
      return '<button class="pickitem" onclick="'+pickOnclick('pickCardio',c.n)+'">'+c.i+' '+c.n+'<span class="grptag">timed</span></button>';
    }).join('');
  }
  ...
  list.forEach(function(e){
    ...
    html+='<button class="pickitem" onclick="'+pickOnclick('pickExercise',e.n)+'">'+e.n+'<span class="grptag">'+GLABEL[e.g[0]]+'</span></button>';
  });
